...
![PrecisionLand in SITL: Gazebo side view and the downward camera](media/precision_land.gif)

*Left: Gazebo side view. The drone takes off 1.4 m from the ground tag, spirals out
until the downward camera finds it, moves over it and descends onto it. Right: the
downward camera as `aruco_tracker` annotates it. Plays at 2x speed.*

| Tag found during the search | |
|---|---|
| ![](media/precision_land_still.png) | The tracker has the tag and draws its pose axes; the mode switches from Search to Approach. |

Recorded in SITL with the `aruco_dual_ids` world and `params.yaml`. A few things to
know when you run it yourself:

- In that world the ground tag is id 1 and 0.5 m wide, but
  `downward_camera_aruco.launch.py` looks for id 0, so the recording ran the tracker
  with `aruco_id: 1, marker_size: 0.5`.
- At touchdown the tag fills the downward camera, the tracker loses it, and the mode
  reports "Target lost during Descend" (Mode Failure) before PX4's land detector
  confirms the landing. The drone is on the tag when that happens.
- The executor also needed the same temporary message-compatibility skip as
  FrontApproach on the v1.16.0 SITL build.
