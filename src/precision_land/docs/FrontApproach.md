...
![FrontApproach in SITL: Gazebo side view and the front camera](media/front_approach.gif)

*Left: Gazebo side view. The drone takes off 3.2 m from the vertical tag and closes
to the 1.25 m hold distance at the tuned 0.15 m/s cap. Right: the front camera as
`aruco_tracker` annotates it. Plays at 3x speed.*

Recorded in SITL with the `aruco_dual_ids` world, `front_camera_aruco.launch.py` and
`front_approach_params.yaml`. On the v1.16.0 SITL build this mode's executor fails
PX4's message-compatibility check, so the recording skipped that check in the
executor temporarily.
...
```bash
ros2 launch precision_land front_approach.launch.py
```
