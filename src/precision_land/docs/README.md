# Precision Land — External Flight Modes
...
|------|---------------|--------|-------------|
| [TakeoffLand](./TakeoffLand.md) | `ros2 launch precision_land takeoff_land.launch.py` | None | Take off, hold, land |
| [TakeoffHold](./TakeoffHold.md) | `ros2 launch precision_land takeoff_hold.launch.py` | None | Take off, hold indefinitely |
| [FrontApproach](./FrontApproach.md) | `ros2 launch precision_land front_approach.launch.py` | Front | Fly toward a front-camera ArUco tag |
| [PrecisionLand](./PrecisionLand.md) | `ros2 launch precision_land precision_land.launch.py` | Down | Search, approach, and land on a downward-camera tag |
| [FrontToPrecisionLand](./FrontToPrecisionLand.md) | `ros2 launch precision_land front_to_precision_land.launch.py` | Front + Down | Approach via front camera, then land via downward camera |
...
ros2 launch precision_land takeoff_land.launch.py
```
...
```bash
ros2 launch precision_land front_approach.launch.py front_pid_kp:=1.0 front_hold_distance:=2.0
```
...
precision_land/
  ...
  PrecisionLandAuto.hpp/cpp  # Arms, takes off, precision lands, and disarms by itself
  BlankMode.hpp/cpp      # Empty external-mode template
  StatePublisher.hpp     # Publishes the mode's state on <node>/state
  TrackingErrorPublisher.hpp  # Publishes commanded-minus-actual position on /tracking_error
  test/sitl_debug_topics.sh   # Headless SITL check that a mode publishes both topics
