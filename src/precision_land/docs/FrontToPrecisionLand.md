...
![FrontToPrecisionLand in SITL: Gazebo view and the front camera](media/front_to_precision_land.gif)

*Left: Gazebo view from the side. The drone climbs, finds the vertical tag with the
front camera, approaches to the 1 m hold distance, then descends straight down onto
the landing tag. Right: the front camera as `aruco_tracker` annotates it. Plays at
2x speed.*

What the recording showed about the current code:

- After takeoff the mode turns to yaw 0 (north), so it only finds a tag that is north
  of the drone. The recording used a copy of `aruco_dual_ids` rotated 90 degrees for
  that reason.
- The final descent uses no camera: the mode drops straight down from where the front
  approach stopped. The "Two Cameras, Two Tags" section below describes a downward
  camera stage that the current code doesn't run.
- After touchdown the mode keeps lowering its setpoint and hadn't finished 40 s later,
  because PX4 never reported the vehicle as landed.
- If a tag detection arrives before PX4 odometry does, the node exits with
  "No messages received". Start the tracker after the mode has registered.
- The executor needed the same temporary message-compatibility skip as FrontApproach
  on the v1.16.0 SITL build.
...
```bash
ros2 launch precision_land front_to_precision_land.launch.py
```
