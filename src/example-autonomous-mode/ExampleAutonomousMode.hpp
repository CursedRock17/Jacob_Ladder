...
/**
 * ExampleAutonomousMode.hpp — Takeoff, Hold, and Land Flight Mode
 *
 * This mode makes the drone:
 *   1. Rise to a low "optical flow" height and pause (so the flow sensor
 * initializes)
...
private:
  ...
  enum class State {
    ...
    InitalTakeoffAltitude, // Initial height for drone to climb without relying
                           // on flow or VIO odom
...
private:
  ...
  float _optical_flow_hold_time =
      3.0f;                      // How long to hover at flow height (seconds)
