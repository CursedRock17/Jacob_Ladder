...
private:
  ...
  // The executor performs a PX4 native takeoff before scheduling this mode, so
  // the vehicle is already airborne on activation and there is no in-mode
  // climb. There is no terminal state either: once the standoff point is
  // reached the mode holds station indefinitely and keeps tracking the drogue,
  // dropping back to Approach if the drogue moves far enough to need a
  // speed-limited transit.
  enum class State {
  ...
  // Transform the cached drogue pose (ranging frame: +x left, +y up, +z
  // forward) into an absolute NED target using the vehicle attitude. Returns
  // false if no valid drogue pose or attitude is available yet.
  bool drogueTargetNed(Eigen::Vector3f &target_ned) const;

  // Standoff point we actually fly to: drogue_standoff_m back along the
  // drone->drogue bearing, at the drogue's altitude. Measured from our own
  // approach line, so the path to it never crosses the drogue and it
  // self-corrects on overshoot. Returns false for the same reasons
  // drogueTargetNed() does.
  ...
  // Helpers
  void publishPathVisualization(const Eigen::Vector3f &from,
                                const Eigen::Vector3f &standoff,
...
private:
  ...
  float _param_drogue_standoff_m =
      3.0f; // Standoff kept back from the drogue [m]
  float _param_carrot_lead_time =
      1.0f; // Setpoint lead ahead of the vehicle [s]
  ...
  float _param_camera_pitch_deg =
      0.0f; // Forward cam mount pitch, +ve = tilted down [deg]
  float _param_drogue_timeout =
      3.0f; // Max age of drogue pose before considered lost [s]
};

// Executor: arm -> PX4 native takeoff -> hand off to the DroneSmoothPlanner
// mode
class DroneSmoothPlannerExecutor : public px4_ros2::ModeExecutorBase {
public:
  DroneSmoothPlannerExecutor(rclcpp::Node &node,
