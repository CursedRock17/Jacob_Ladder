...
DroneSmoothPlanner::DroneSmoothPlanner(rclcpp::Node &node)
  ...
  _drogue_pose_sub = _node.create_subscription<geometry_msgs::msg::PoseStamped>(
      "/tag_detections", qos,
      std::bind(&DroneSmoothPlanner::droguePoseCallback, this,
                std::placeholders::_1));

  // Path publisher for Foxglove visualization
  _path_viz_pub =
      _node.create_publisher<nav_msgs::msg::Path>("/trajectory_path_viz", qos);
...
void DroneSmoothPlanner::loadParameters() {
  ...
  _node.get_parameter("final_waypoint_tolerance_m",
...
void DroneSmoothPlanner::droguePoseCallback(
    const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  _drogue_pose = Eigen::Vector3f(static_cast<float>(msg->pose.position.x),
...
void DroneSmoothPlanner::updateSetpoint(float dt_s) {
  ...
  bool drogue_expired = !_drogue_valid || (now - _drogue_timestamp).seconds() >
                                              _param_drogue_timeout;

  switch (_state) {

  case State::Search: {
    RCLCPP_INFO_THROTTLE(
        _node.get_logger(), *_node.get_clock(), 3000,
  ...
  case State::Approach: {
    ...
    // distance the target lands behind us and the same setpoint commands a
    // retreat.
    ...
    if (!drogueTargetNed(drogue_ned) || !drogueStandoffNed(standoff_ned)) {
      ...
      _trajectory_setpoint->updatePosition(
    ...
    // Error printed per-axis (N/E/D, +D = standoff is below us) rather than as
    // a magnitude, so a ground test can see which axis is off. Tolerance shown
    // too.
    RCLCPP_INFO_THROTTLE(
        ...
        _param_final_waypoint_tolerance_m, drogue_ned.x(), drogue_ned.y(),
        drogue_ned.z(), pos.x(), pos.y(), pos.z());
    ...
    if (withinTolerance(to_standoff, _param_final_waypoint_tolerance_m)) {
      RCLCPP_INFO(
          ...
          _param_drogue_standoff_m, to_standoff.x(), to_standoff.y(),
          to_standoff.z());
      switchToState(State::Hover);
      break;
    }

    // Carrot setpoint: a single waypoint placed one lead-time ahead of the
    // vehicle along the bearing to the standoff point, never past it. PX4's
    // position controller closes the loop; the velocity feed-forward caps
    // approach speed at max_velocity, since offboard setpoints bypass
    // mission-mode trajectory shaping and would otherwise be limited only by
    // MPC_XY_VEL_MAX.
    ...
    // Taper the feed-forward inside the lead distance so the vehicle settles
    // onto the standoff point instead of overshooting it toward the drogue.
    const float speed =
        (full_lead > 0.0f)
            ? _param_max_velocity * std::min(1.0f, distance / full_lead)
            : 0.0f;

    // Yaw tracks the drogue itself so the camera stays on target during
    // transit.
    const float yaw =
        std::atan2(drogue_ned.y() - pos.y(), drogue_ned.x() - pos.x());
  ...
  case State::Hover: {
    // Station-keeping, not a timed hold: the mode stays here indefinitely and
    // never completes, so the vehicle holds the standoff until the pilot takes
    // over.
    if (drogue_expired) {
      RCLCPP_WARN(_node.get_logger(), "Drogue lost — returning to search");
      switchToState(State::Search);
    ...
    // The drogue can keep moving, so the standoff is re-derived every tick here
    // too and commanded directly. Small drifts are corrected by PX4's position
    // controller without leaving this state — this is what keeps us tracking a
    // drifting drogue.
    Eigen::Vector3f drogue_ned;
    Eigen::Vector3f standoff_ned;
    if (!drogueTargetNed(drogue_ned) || !drogueStandoffNed(standoff_ned)) {
    ...
    // Hand back to Approach once the drogue has moved far enough that closing
    // the gap deserves the speed-limited carrot. Commanding a distant position
    // setpoint directly would let the controller pick its own (much faster)
    // transit speed, which is exactly what the carrot exists to prevent. The 2x
    // band gives hysteresis so the two states cannot chatter at the tolerance
    // boundary.
    if (!withinTolerance(to_standoff,
                         2.0f * _param_final_waypoint_tolerance_m)) {
      RCLCPP_INFO(
          _node.get_logger(),
    ...
    _trajectory_setpoint->update(
        px4_ros2::TrajectorySetpoint{}.withPosition(standoff_ned).withYaw(yaw));
...
bool DroneSmoothPlanner::drogueTargetNed(Eigen::Vector3f &target_ned) const {
  ...
  // Attitude may not have arrived yet — a zero/degenerate quaternion is
  // unusable
  ...
  // Ranging (+x left, +y up, +z forward) -> body FRD, assuming the camera
  // optical axis is body-forward. If the forward cam is mounted with a fixed
  // pitch, tilt the vector by that mount angle (positive camera_pitch_deg =
  // tilted down).
  const Eigen::Vector3f frd_cam(_drogue_pose.z(), -_drogue_pose.x(),
                                -_drogue_pose.y());
  const float pitch_rad =
      _param_camera_pitch_deg * static_cast<float>(M_PI) / 180.0f;
...
bool DroneSmoothPlanner::drogueStandoffNed(
    Eigen::Vector3f &standoff_ned) const {
  Eigen::Vector3f drogue_ned;
  ...
  // drogue's altitude. Because it is measured from our own approach line rather
  // than a fixed heading, the carrot toward it never crosses the drogue, and it
  // self- corrects: drift inside the standoff distance puts the target behind
  // us, so the same setpoint drives a retreat back out to the ring.
  const Eigen::Vector3f pos = _vehicle_local_position->positionNed();
  const Eigen::Vector2f to_drogue_xy(drogue_ned.x() - pos.x(),
                                     drogue_ned.y() - pos.y());
  ...
  if (distance_xy < 1e-3f) {
    // Directly above/below the drogue — no horizontal bearing to back off
    // along.
...
bool DroneSmoothPlanner::withinTolerance(const Eigen::Vector3f &error_ned,
                                         float band_m) const {
  return std::abs(error_ned.x()) <= band_m &&
         std::abs(error_ned.y()) <= band_m && std::abs(error_ned.z()) <= band_m;
}

// Publish vehicle -> standoff -> drogue as a nav_msgs::Path for Foxglove, so
// the standoff gap is visible as the final leg of the line.
void DroneSmoothPlanner::publishPathVisualization(
    const Eigen::Vector3f &from, const Eigen::Vector3f &standoff,
...
void DroneSmoothPlanner::switchToState(State state) {
  if (_state == state)
    return;
  RCLCPP_INFO(_node.get_logger(), "State: %s -> %s", stateName(_state).c_str(),
...
// ── Executor: arm -> PX4 native takeoff -> schedule DroneSmoothPlanner mode ──

DroneSmoothPlannerExecutor::DroneSmoothPlannerExecutor(
  ...
  // takeoff_height is declared by the mode, which is constructed first; reuse
  // it as the native takeoff altitude. Consider takeoff done within 0.2 m (NED
  // z is neg).
  ...
  // takeoff()'s completion callback does not fire reliably below
  // MIS_TAKEOFF_ALT, so watch local position z directly as well — same
  // workaround as TakeoffLand.
  _local_pos_sub =
      _node.create_subscription<px4_msgs::msg::VehicleLocalPosition>(
          ...
          [this](const px4_msgs::msg::VehicleLocalPosition::SharedPtr msg) {
            if (_in_takeoff && !_takeoff_complete &&
              ...
              RCLCPP_INFO(
                  _node.get_logger(),
                  "Takeoff altitude reached (z=%.2f) — handing off to mode",
                  msg->z);
...
void DroneSmoothPlannerExecutor::onActivate() {
  RCLCPP_INFO(_node.get_logger(),
              "DroneSmoothPlanner executor — arm, PX4 native takeoff to %.2f "
              "m, then approach",
...
// Sequential state machine driven by PX4 result callbacks: arm -> takeoff ->
// mode
void DroneSmoothPlannerExecutor::runState(State state,
  ...
  case State::TakingOff:
    ...
    _in_takeoff = true;
    _takeoff_complete = false;
    // Whichever of this callback and the local-position watcher fires first
    // wins.
    takeoff(
  ...
  case State::Approaching:
    ...
    scheduleMode(ownedMode().id(), [this](px4_ros2::Result r) {
      RCLCPP_INFO(_node.get_logger(), "DroneSmoothPlanner mode ended (%s)",
                  resultToString(r));
    });
    break;
  }
