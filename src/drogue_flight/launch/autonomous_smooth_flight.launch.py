...
def generate_launch_description():
    ...
    setpoint_rate_hz_launch = DeclareLaunchArgument(
        "setpoint_rate_hz",
        default_value="20",
    )
    s_curve_steepness_launch = DeclareLaunchArgument(
        "s_curve_steepness", default_value="4.0"
    )
    waypoint_tolerance_m_launch = DeclareLaunchArgument(
        "waypoint_tolerance_m", default_value="0.15"
    )
    max_velocity_launch = DeclareLaunchArgument("max_velocity", default_value="0.50")
    max_acceleration_launch = DeclareLaunchArgument(
        "max_acceleration", default_value="0.35"
    )
    ...
    ros2bag_node = ComposableNodeContainer(
        ...
        composable_node_descriptions=[
            ComposableNode(
                ...
                name="recorder",
                parameters=dump_params(
                    os.path.join("./drogue_flight/config/", "recorder_params.yaml"),
                    "recorder",
                ),
