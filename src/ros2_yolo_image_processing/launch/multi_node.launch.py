...
def generate_launch_description():
    ...
    min_score_threshold_launch = DeclareLaunchArgument(
        "min_score_threshold", default_value="0.25"
    )
    min_iou_threshold_launch = DeclareLaunchArgument(
        "min_iou_threshold", default_value="0.45"
    )
    ...
    rviz = Node(package="rviz2", executable="rviz2", name="rviz_node", output="screen")
        ...
        composable_node_descriptions=[
            ComposableNode(
                ...
                parameters=dump_params(
                    os.path.join(config_dir, "recorder_params.yaml"), "recorder"
                ),
