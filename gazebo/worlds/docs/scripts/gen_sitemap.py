...
my_parser = argparse.ArgumentParser(
    description="Generate sitemap for all markdown files in directory (default to main for output)"
)
# Add the arguments
...
my_parser.add_argument(
    "-d", "--date", action="store_true", help="generate date information"
)
my_parser.add_argument(
    "-o",
    "--output",
...
for subdir, dirs, files in os.walk(dir_name, topdown=True):
    ...
    if any(
        f"/{inc_dir}/" in subdir or f"\\{inc_dir}\\" in subdir
    ...
    if any(
        f"/{ex_dir}/" in subdir or f"\\{ex_dir}\\" in subdir for ex_dir in exclude_dirs
    ):
        continue
